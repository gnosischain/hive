...
var Tests = []test.Spec{
	...
	&CancunBaseSpec{
		...
		BaseSpec: test.BaseSpec{
			...
			Verify that all transactions are included across the produced payloads.
			On Gnosis MAX_BLOBS_PER_BLOCK=2, so MAX-1 equals 1 and the two batches
			are indistinguishable: 8 single-blob txs pack into 4 full payloads.
		...
		TestSequence: TestSequence{
			...
			// Use a high blob gas cap because on Gnosis each full payload
			// (MAX-TARGET=1 excess) bumps price by ~1.125x, so several
			// consecutive full payloads need headroom above MIN_DATA_GASPRICE.
			SendBlobTransactions{
				TransactionCount:              5,
			...
			// 8 single-blob txs from one account, sequential nonces, pack
			// MAX=2 per payload = 4 full payloads.
			NewPayloads{
				PayloadCount:              4,
				ExpectedIncludedBlobCount: cancun.MAX_BLOBS_PER_BLOCK,
			},
	...
	&CancunBaseSpec{
		...
		BaseSpec: test.BaseSpec{
			...
			using account A.
			Using same account, and an increased nonce from the previously sent
			transactions, send a single 2-blob transaction.
			Verify that all transactions are included across the produced payloads.
			On Gnosis MAX_BLOBS_PER_BLOCK=2, so MAX-1=1: 5 single-blob txs followed
			by a 2-blob tx pack (in nonce order) as 2,2,1,2 blob payloads, since
			the 2-blob tx cannot share a block with another blob tx.
			All transactions have sufficient data gas price to be included any
			of the payloads.
			`,
			MainFork: config.Cancun,
		...
		TestSequence: TestSequence{
			// First send the cancun.MAX_BLOBS_PER_BLOCK-1 blob transactions.
			// Use a high blob gas cap so price increases across consecutive
			// full payloads do not exclude later txs (see Single Blob test).
			SendBlobTransactions{
				TransactionCount:              5,
				BlobsPerTransaction:           cancun.MAX_BLOBS_PER_BLOCK - 1,
				BlobTransactionMaxBlobGasCost: big.NewInt(1e11),
			...
			// 5 single-blob txs pack 2 per payload until the last one runs
			// solo because the next tx (2 blobs) won't fit alongside it.
			NewPayloads{
				PayloadCount:              2,
				ExpectedIncludedBlobCount: cancun.MAX_BLOBS_PER_BLOCK,
			},
			...
			// Finally the dual-blob transaction in its own payload.
			NewPayloads{
				PayloadCount:              1,
				ExpectedIncludedBlobCount: cancun.MAX_BLOBS_PER_BLOCK,
			},
	...
	&CancunBaseSpec{
		...
		TestSequence: TestSequence{
			...
			SendBlobTransactions{ // Blob ID 1
				TransactionCount:              1,
				BlobTransactionMaxBlobGasCost: big.NewInt(1e10),
			...
			SendBlobTransactions{ // Blob ID 3
				TransactionCount:              1,
				BlobTransactionMaxBlobGasCost: big.NewInt(1e12),
	...
	&CancunBaseSpec{
		BaseSpec: test.BaseSpec{
			...
			Tests VersionedHashes in Engine API NewPayloadV3 where the array
			is out of order.
			On Gnosis TARGET_BLOBS_PER_BLOCK=1, so a single tx with
			MAX_BLOBS_PER_BLOCK=2 blobs is used to provide >1 hash to reorder.
			`,
			MainFork: config.Cancun,
		},
		TestSequence: TestSequence{
			...
			NewPayloads{
				...
				NewPayloadCustomizer: &helper.BaseNewPayloadVersionCustomizer{
					PayloadCustomizer: &helper.CustomPayloadData{
						VersionedHashesCustomizer: &VersionedHashes{
							Blobs: helper.GetBlobListByIndex(helper.BlobID(cancun.MAX_BLOBS_PER_BLOCK-1), 0),
						},
					},
					ExpectInvalidStatus: true,
				},
