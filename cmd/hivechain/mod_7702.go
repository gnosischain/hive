...
func (m *mod7702) authorizeCode(ctx *genBlockContext) error {
	...
	gas, err := core.IntrinsicGas(txdata.Data, txdata.AccessList, txdata.AuthList, sender.addr, &txdata.To, uint256.NewInt(0), ctx.ChainConfig().Rules(ctx.Number(), true, ctx.Timestamp()))
	if err != nil {
		panic(err)
	}
	txdata.Gas = gas
