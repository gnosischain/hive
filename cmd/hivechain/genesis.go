...
var (
	...
	blockRewardContractCode = common.FromHex("0x604060005260606020526000604052600060605260806000f3")
...
func (cfg *generatorConfig) createGenesis() *core.Genesis {
	...
	// Keep the genesis header stable when setDifficulty later fills in TTD at
	// the merge transition. Pre-merge AuRa genesis headers carry a signature;
	// merged-from-genesis headers retain the standard nonce/mixHash layout.
	if g.Config.MergeNetsplitBlock == nil || g.Config.MergeNetsplitBlock.Sign() > 0 {
		g.AuRaSeal = make([]byte, 65)
	}
